#!/usr/bin/env python3
"""
Pull real daily contribution counts for GH_PROFILE_USER from the public
github-contributions-api.jogruber.de mirror (no token needed) and write
data/contributions.json with the raw days plus derived stats (current
streak, longest streak, best day, monthly totals).

Run daily by .github/workflows/update-contrib-graph.yml.
"""
import datetime
import json
import os
...
USERNAME = os.environ.get("GH_PROFILE_USER", "Ashrokss")
...
def fetch_days():
    ...
    days = [{"date": d["date"], "count": d["count"]} for d in payload["contributions"]]
