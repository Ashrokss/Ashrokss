...
GitHub-style contribution heatmap SVG: a terminal window frame around the
classic 53-week x 7-day calendar, a one-shot diagonal reveal (CSS keyframes,
plays on load then freezes), a Less->More legend, and a real stats footer.

Run by .github/workflows/update-contrib-graph.yml after fetch_contributions.py.
"""
import datetime
import json
import os
...
# classic GitHub green ramp: empty -> brightest
...
""".strip()
    ...
    parts = [
        ...
        f'fill="none" stroke="{FRAME}" stroke-width="1" stroke-opacity="0.8"/>',
    ...
    parts.append(f'<text x="{canvas_w/2}" y="{TITLEBAR_H/2 + 4}" fill="{MUTED}" font-size="12" '
                 f'text-anchor="middle">ashish@github: ~/contributions --graph</text>')
    ...
    sep_y = leg_y + CELL + 14
    parts.append(f'<line x1="0" y1="{sep_y}" x2="{canvas_w}" y2="{sep_y}" stroke="{FRAME}" stroke-opacity="0.5"/>')
